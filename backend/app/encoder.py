...
def encode_application(req: LoanApplicationRequest, override_dti: float | None = None) -> Tuple[np.ndarray, Dict[str, float]]:
    ...
    emp_years = req.get_effective_employment_years()
    effective_dti = override_dti if override_dti is not None else (req.dti if req.dti is not None else 15.0)
    ...
    raw_dict: Dict[str, float] = {
        ...
        "term": float(req.term),
        "emp_length": float(emp_years),
        "annual_inc": float(annual_income),
        "dti": float(effective_dti),
